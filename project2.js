...
class MeshDrawer {
	...
	draw(trans) {
		...
		console.log("Blend Factor in Shader:", gl.getUniform(this.prog, gl.getUniformLocation(this.prog, 'blendFactor')));
    	console.log("Active Textures:", gl.TEXTURE0, gl.TEXTURE1);
	...
	setBlendFactor(factor) {
		gl.useProgram(this.prog);
		const blendFactorLoc = gl.getUniformLocation(this.prog, 'blendFactor');
		gl.uniform1f(blendFactorLoc, factor);
	}
...
function updateLightPos() {
    ...
    gl.uniform3fv(meshDrawer.lightPosLoc, [lightX, lightY, 1.0]);
}
...
function SetBlendFactor(param) {
    const blendFactor = param.value / 100.0; 
    meshDrawer.setBlendFactor(blendFactor);
	console.log("Blend Factor in Shader:", blendFactor);
    DrawScene();
}
...
const meshFS = `
	...
	void main() {
		...
		vec4 blendedColor = mix(texColor, texColor2, blendFactor);
        ...
        if (showTex && enableLighting) {
            ...
            gl_FragColor = vec4(blendedColor.rgb * totalLight, blendedColor.a);
